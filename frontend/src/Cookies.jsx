...
function CookiesBasics() {
  ...
  const handleSetCookie = async () => {
    try {
      const res = await fetch(
        "http://localhost:3000/api/cookies/set",
        fetchOptions
      );
  ...
  const handleGetCookie = async () => {
    try {
      const res = await fetch(
        "http://localhost:3000/api/cookies/get",
        fetchOptions
      );
      const data = await res.json();
      setCookiesInfo(data);
  ...
  return (
    ...
    >
      <h2 style={{ marginTop: 0, color: "#1e293b" }}>
        🍪 Module 1: HTTP Cookie Basics
      </h2>
      <p style={{ color: "#64748b" }}>
        Test setting, reading, and clearing raw cookies sent over the network
        headers.
      </p>
